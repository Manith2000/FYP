...
# urdfObj = "setup/objects/ycb_objects/YcbStrawberry/model.urdf"
urdfObj = "setup/objects/ycb_objects/YcbBanana/model.urdf"
# urdfObj = "setup/objects/ycb_objects/sphere_small.urdf"
# urdfObj = "setup/objects/ycb_objects/YcbTennisBall/model.urdf"
# urdfObj = "setup/objects/ycb_objects/YcbMustardBottle/model.urdf"
# urdfObj = "setup/objects/ycb_objects/YcbTomatoSoupCan/model.urdf"
globalScaling = 1.0

# objStartPos = [] #Tennis ball
# objStartOrientation = pb.getQuaternionFromEuler([0, 0, np.pi/2]) #Tennis ball   vary Rz

# objStartPos = [0.52, 0.01, 0.085] #Marble
# objStartOrientation = pb.getQuaternionFromEuler([0, 0, np.pi/2]) #Tennis ball   vary Rz

# objStartPos = [0.54, 0, 0.077] #Strawberry
# objStartOrientation = pb.getQuaternionFromEuler([0, 0, 0]) #Strawberry

objStartPos = [0.54, 0, 0.09] #Banana  vary y between  -0.02 -- 0.06
objStartOrientation = pb.getQuaternionFromEuler([0, 0, np.pi/2]) #Banana  vary Rx 


# objStartPos = [0.54, 0, 0.09] #CHips can  vary y between  -0.02 -- 0.06
# objStartOrientation = pb.getQuaternionFromEuler([np.pi/2, 0, 0]) #Banana  vary Rx


# objStartPos = [0.54, 0, 0.09] #tomato Can  vary y between  -0.02 -- 0.06
# objStartOrientation = pb.getQuaternionFromEuler([np.pi/2, 0, 0]) #Banana  vary Rx
...
while True:
    ...
    if t > 15 and t < 45:
        ...
        # Grasping
        rob.go(pos, width=0.05, gripForce=gripForce)
    elif t == 45:
        ...
        # Plot each recorded image from the two digits
        plt.imshow(tactileColorL)
        # plt.imshow(tactileColorR)
        # plt.imshow(tactileDepthL)
        # plt.imshow(tactileDepthR)
        plt.axis('off')
        imgid += 1
        plt.savefig(str(imgid) + 'c' + '.png', bbox_inches='tight')
        # plt.show()
        ...
        # plt.imshow(tactileDepthR)
        plt.axis('off')
        plt.savefig(str(imgid) + 'd' + '.png', bbox_inches='tight')
        # plt.show()
    ...
    # #Marble
    #     # objRestartPos = [0.51, 0.035, 0.088] z: 0.084 to 0.108
    #     objRestartPos = [0.51, 0.035, 0.084 + 0.021*np.random.random()]

    #     objRestartOrientation = pb.getQuaternionFromEuler(
    #         [2 * np.pi * np.random.random(), 2 * np.pi * np.random.random(), 2 * np.pi * np.random.random()]
    #     )

        # #STRAWBERRY
        # objRestartPos = [0.5, 0.04, 0.09] 
        ...
        #BANANA
        objRestartPos = [0.5, 0, 0.095] 

        objRestartOrientation = pb.getQuaternionFromEuler(
            [2 * np.pi * np.random.random(), 0, np.pi/2]
        )

        #  #Mustard
        # objRestartPos = [0.51, 0.035, 0.088]

        # objRestartOrientation = pb.getQuaternionFromEuler([2*np.pi* np.random.random(), 0, 0])
